...
class DevSession(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if repository:
                ...
                if supplied_path and supplied_path != repository.working_directory:
                    raise ValidationError(
                        "Session working directory must use the exact registered value."
                    )
                vals["working_directory"] = repository.working_directory
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            if repository and {"repository_id", "working_directory"}.intersection(vals):
                ...
                if supplied_path != repository.working_directory:
                    raise ValidationError(
                        "Session working directory must use the exact registered value."
                    )
                vals["working_directory"] = repository.working_directory
    ...
    def _check_registry_consistency(self):
        for record in self:
            ...
            if (
                not record.working_directory
                or record.working_directory != record.repository_id.working_directory
                or record.working_directory
                != record.repository_id.canonical_remote_path
            ):
                raise ValidationError(
                    "Session working directory must exactly match both canonical "
