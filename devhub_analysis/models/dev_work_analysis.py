...
class DevWorkAnalysis(models.Model):
    ...
    @api.depends(
        "status",
        "work_item_id",
        "work_item_id.current_phase",
        "problem_summary",
    )
    def _compute_step_guide(self):
        super()._compute_step_guide()
    ...
    def _step_guide_payload(self):
        ...
        if not work:
            return {
                ...
                "message": "Complete these first: create or open a Work Item in the Dev Hub Work app, then link this Analysis.",
            }
        if self.status == "accepted":
            return {
                "state": "done",
                "title": "Analysis accepted — next: Plan",
                "message": (
                    "This analysis is accepted. Open the Dev Hub Plan app and create/submit a plan "
                    "for work item '%s' (move the work item to Planning if needed)."
                    % (work.display_name,)
                ),
            }
        if work.current_phase not in ("analyzing", "registered", "triage", "received", "planning"):
            return {
                "state": "blocked",
                "title": "Wrong work-item phase",
                "message": (
                    "Complete these first: set Work Item '%s' to Analyzing "
                    "(current phase: %s), then accept this analysis."
                    % (work.display_name, work.current_phase)
                ),
            }
        if not (self.problem_summary or "").strip():
            return {
                "state": "blocked",
                "title": "Fill analysis content",
                "message": "Complete these first: write Problem Summary (and findings), then click Accept Analysis.",
            }
        if self.status in ("draft", "generated", "reviewed"):
            return {
                "state": "ready",
                "title": "Ready to accept analysis",
                "message": (
                    "Review the Request/Findings tabs. When complete, click Accept Analysis. "
                    "Work item should be in Analyzing phase."
                ),
            }
        return {
            "state": "info",
            "title": "Analysis step",
            "message": "Status: %s. Use New Revision if you need another analysis version." % self.status,
        }
    ...
    def action_accept(self):
        ...
        if not self.work_item_id:
            missing.append("Link a Work Item (Dev Hub Work app)")
        elif self.work_item_id.current_phase != "analyzing":
            missing.append(
                "Move Work Item '%s' to Analyzing (current: %s)"
                % (self.work_item_id.display_name, self.work_item_id.current_phase)
            )
        if not (self.problem_summary or "").strip():
            missing.append("Fill Problem Summary on this Analysis")
        if self.status not in ("draft", "generated", "reviewed"):
            missing.append("Use a Draft/Generated/Reviewed analysis revision (not %s)" % self.status)
