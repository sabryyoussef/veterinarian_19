...
class DevMachine(models.Model):
    ...
    def action_verify_tailscale_destination(self, dry_run=False):
        raise UserError(
            "Install and enable Dev Hub Infrastructure (devhub_infrastructure) "
            "to run Tailscale/SSH destination verification."
        )
...
class DevProject(models.Model):
    ...
    def action_open_test_environment_url(self):
        self.ensure_one()
        env = self.default_environment_id
        if not env or not env.url:
            raise ValidationError("No default environment URL is configured.")
        return {
    ...
    def action_open_default_repository_path(self):
        self.ensure_one()
        repo = self.default_repository_id
        if not repo or not repo.working_directory:
            raise ValidationError("No default repository path is configured.")
        return {
            "type": "ir.actions.client",
            "tag": "display_notification",
            "params": {
                "title": "Repository path",
                "message": repo.working_directory,
                "sticky": True,
                "type": "info",
            },
        }
...
class DevPolicy(models.Model):
    ...
    def create(self, vals_list):
        # During module XML load, reuse an existing scoped policy instead of
        # failing when an orphan row already occupies the unique scope
        # (e.g. prior seed without ir.model.data linkage).
        if self.env.context.get("install_mode") or self.env.context.get("module"):
            ...
            for vals in vals_list:
                ...
                domain = [
                    ...
                    ("environment_id", "=", vals.get("environment_id") or False),
                ]
                ...
                if existing:
                    write_vals = {
                        ...
                        if key not in ("project_id", "environment_id")
                    }
                    if write_vals:
...
class DevDashboard(models.Model):
    ...
    work_item_count = fields.Integer(compute="_compute_counts")
    ...
    def _compute_counts(self):
        ...
        Session = self.env["dev.session"] if "dev.session" in self.env else None
        for record in self:
            record.work_item_count = (
                Work.search_count([("active", "=", True)]) if Work is not None else 0
            )
            record.recent_session_count = Session.search_count([]) if Session is not None else 0
            record.active_session_count = (
                Session.search_count([("state", "in", ["started", "in_progress", "resumed"])])
                if Session is not None
                else 0
            )
            record.paused_session_count = (
                Session.search_count([("state", "=", "paused")]) if Session is not None else 0
            )
    ...
    def action_work_items(self):
        return self._open_action("devhub_work.action_dev_work_item")

    def action_sessions(self):
        return self._open_action("devhub_session.action_dev_session")

    def action_active_sessions(self):
        return self._open_action(
            "devhub_session.action_dev_session",
            [("state", "in", ["started", "in_progress", "resumed"])],
        )

    def action_paused_sessions(self):
        return self._open_action(
            "devhub_session.action_dev_session", [("state", "=", "paused")]
        )

    def action_projects(self):
        return self._open_action("devhub_core.action_dev_project")

    def action_environments(self):
        return self._open_action("devhub_core.action_dev_environment")

    def action_machines(self):
        return self._open_action("devhub_core.action_dev_machine")

    def action_clients(self):
        return self._open_action("devhub_core.action_dev_client")
