...
from odoo.addons.devhub_execution.models.dev_git_constants import SHA1_RE
from odoo.addons.devhub_git.models.dev_git_commit import _canonical_hash
...
class DevExecutionWorkspaceDeploy(models.Model):
    ...
    def _require_deploy_approver(self, production=False):
        group = (
            "devhub_core.group_dev_hub_production_approver"
            if production
            else "devhub_core.group_dev_hub_deploy_approver"
        )
    ...
    def _verify_sha_on_branch(self, merge_sha, branch, target=None):
        """Prove merge SHA is the tip of the required protected branch.

        Non-production: verify via ``git ls-remote`` against the registered remote.
        Production: remain closed unless an explicit simulated test context is set.
        """
        if self.env.context.get("dev_deploy_skip_remote"):
            return True
        if not SHA1_RE.fullmatch(merge_sha or ""):
            raise UserError("Invalid merge SHA for deploy verification.")
        if not branch or not re.fullmatch(r"[A-Za-z0-9._/-]{1,200}", branch) or ".." in branch:
            raise UserError("Invalid protected branch name for deploy verification.")
        if target is not None and target.target_kind == "production":
            raise UserError(
                "Remote SHA verification for production requires an allowlisted runner "
                "preflight (set test context dev_deploy_skip_remote for simulated flows)."
            )
        remote = (self.repository_id.git_remote or "").strip()
        if not remote:
            raise UserError("Repository git remote is required for deploy SHA verification.")
        ref = "refs/heads/%s" % branch
        try:
            result = subprocess.run(
                ["/usr/bin/git", "ls-remote", remote, ref],
                check=False,
                capture_output=True,
                text=True,
                timeout=60,
                env={
                    "PATH": "/usr/bin:/bin",
                    "HOME": os.path.expanduser("~") or "/home/sabry",
                    "LANG": "C",
                    "LC_ALL": "C",
            ...
            raise UserError("Unable to verify protected branch tip via git ls-remote.") from exc
        if result.returncode:
            raise UserError(
                "Protected branch tip lookup failed for %s on %s." % (branch, remote)
            )
        tip = ""
        for line in (result.stdout or "").splitlines():
            parts = line.split()
            if len(parts) >= 2 and parts[1] == ref:
                tip = parts[0].strip().lower()
                break
        if not tip or tip != merge_sha.lower():
            raise UserError(
                "Merge SHA %s is not the tip of protected branch %s (remote tip %s)."
                % (merge_sha, branch, tip or "unknown")
            )
        return True

    def _run_allowlisted_staging_runner(self, approval, lease_token):
        """Execute Test/Staging-only runner verbs against the allowlisted profile."""
        target = approval.target_id
        if target.target_kind != "staging" or not target.non_production:
            raise AccessError("Live runner execution is limited to non-production staging targets.")
        runner = _assert_runner_path(target.runner_profile_reference)
        if not os.path.isfile(runner) or not os.access(runner, os.X_OK):
            raise UserError("Allowlisted staging runner is missing or not executable.")
        db = (target.database_identifier or "").strip()
        if db != "pet_spot_elsahel_test" or any(
            marker in db.lower() for marker in ("prod", "production")
        ):
            raise AccessError("Live runner refused: database is not the allowlisted Test DB.")
        modules = ",".join(
            line.strip()
            for line in (target.module_allowlist or "").splitlines()
            if line.strip() and not line.strip().startswith("#")
        )
        if not modules:
            raise UserError("Deploy target module allowlist is empty.")
        # Docs-only / code-sync deploys: preflight → deploy_code → healthcheck → smoke.
        # Module upgrade is opt-in via context to avoid unintended -u outside allowlist.
        verbs = ["preflight", "deploy_code", "healthcheck", "smoke"]
        if self.env.context.get("dev_deploy_upgrade_modules"):
            verbs.insert(2, "upgrade_modules")
        logs = []
        for verb in verbs:
            cmd = [
                runner,
            ...
            try:
                ...
                raise UserError("Allowlisted staging runner failed to execute.") from exc
            logs.append(
                {
                    "verb": verb,
            ...
            if completed.returncode:
                raise UserError(
                    "Allowlisted staging runner verb %s failed (rc=%s)."
    ...
    def create_deploy_approval(self, target, requester):
        ...
        self._verify_sha_on_branch(
            merge_record.merge_sha, target.required_protected_branch, target=target
        )
    ...
    def execute_approved_deploy(self, approval):
        ...
        backup_ref = ""
        runner_logs = []
        if production and execute_runner and not simulate:
            raise UserError(
                "Live production runner execution is disabled in this control-plane build; "
        ...
        if simulate:
            ...
            # Production remains fail-closed without simulate/skip contexts.
            if not self.env.context.get("dev_deploy_skip_remote"):
            ...
            # Real Test/Staging path: allowlisted runner with confirm-live.
            try:
                runner_logs = self._run_allowlisted_staging_runner(approval, lease_token)
                backup_ref = "runner://staging/%s" % lease_token[:12]
                result_state = "succeeded"
                self.write({"state": "deployed_staging_reviewed"})
            except UserError:
                result_state = "failed_safely"
                self.write({"state": "deploy_staging_failed_safely"})
                raise
        ...
        if runner_logs:
            note = "%s verbs=%s" % (
                note,
                ",".join(item["verb"] for item in runner_logs),
            )
        self.env["dev.deploy.approval.event"].sudo().with_context(dev_deploy_event=True).create(
            {
                "approval_id": approval.id,
                "event_type": "consumed",
                "note": note,
        ...
        if result_state == "succeeded" and not production:
            self.env["dev.deploy.promotion.evidence"].sudo().create(
                {
