...
from odoo.addons.devhub_execution.models.dev_git_constants import BRANCH_RE, SHA1_RE  # noqa: E402
FORBIDDEN_ROOTS = (
    "/home/sabry",
    "/root",
    "/etc",
...
class DevExecutionWorkspace(models.Model):
    ...
    def action_create_and_start_isolated_session(self, client, user=None):
        """Create an isolated session only when the workspace is Ready, then start it.

        Correct lifecycle:
        prepare proposal → confirm → Ready → this helper → implement handoff.
        """
        self.ensure_one()
        if self.state != "ready":
            raise UserError(
                "Isolated session start requires workspace Ready. "
                "Confirm preparation before creating the session."
            )
        self._validate_physical()
        user = user or self.env.user
        session = self.env["dev.session"].create(
            {
                "name": "Isolated session for %s" % self.name,
    ...
    def action_start(self):
        ...
        if workspace:
            ...
            if workspace.state != "ready":
                raise UserError(
                    "The isolated workspace is not Ready. "
                    "Correct order: prepare workspace → confirm → Ready → "
                    "create/link isolated session → action_start → then implement."
                )
