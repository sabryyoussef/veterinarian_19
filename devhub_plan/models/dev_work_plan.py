...
class DevWorkPlan(models.Model):
    ...
    def _missing_before_submit(self):
        self.ensure_one()
        missing = []
        ...
        if work.current_phase != "planning":
            missing.append(
                "Move Work Item '%s' to Planning (current: %s)"
                % (work.display_name, work.current_phase)
            )
        if "dev.work.analysis" in self.env and hasattr(work, "analysis_ids"):
            accepted = work.analysis_ids.filtered(lambda a: a.status == "accepted")
            analysis_id = getattr(self, "analysis_id", False)
            if not accepted and not analysis_id:
                missing.append(
                    "Accept an Analysis first (Dev Hub Analysis app) for this Work Item"
                )
            elif analysis_id and analysis_id.status != "accepted":
                missing.append(
                    "Link an Accepted Analysis (current analysis status: %s)"
                    % analysis_id.status
                )
        if self.status != "draft":
            missing.append("Plan must be Draft to submit (current: %s)" % self.status)
        if not self._is_complete():
            missing.append(
                "Fill all required Plan fields (goal, scope, changes, tests, rollback, …)"
            )
        if not self.step_ids:
            missing.append("Add at least one Plan Step")
        return missing
    ...
    def _step_guide_payload(self):
        ...
        if self.status == "approved":
            return {
                "state": "done",
                "title": "Plan approved — next: Session / Execution",
                "message": (
                    "Exact-hash plan is approved. Open Dev Hub Sessions or Execution to "
                    "prepare the workspace, then Git / GitHub / Deploy as needed."
                ),
            }
        if self.status == "awaiting_approval":
            return {
                ...
                "message": (
                    "Open Dev Hub Approval (or use Approve Exact Hash on this plan). "
                    "Approver must differ from the merge requester later in the path."
                ),
            }
        missing = self._missing_before_submit()
        if missing:
            return {
                "state": "blocked",
        ...
        return {
            "state": "ready",
            "title": "Ready to request approval",
            "message": "Plan looks complete. Click Request Approval when ready.",
        }
    ...
    def action_submit_for_approval(self):
        self.ensure_one()
        self._raise_complete_first(self._missing_before_submit())
        self._refresh_hash()
