...
WORKSPACE_STATES = [
    ...
    ("deploy_production_failed_safely", "Deploy Production Failed Safely"),
    ("deploy_production_uncertain", "Deploy Production Uncertain"),
...
class DevExecutionWorkspace(models.Model):
    ...
    def create_proposal(self, work):
        ...
        # approved: normal start of isolated implementation
        # ready_for_review / completed: allow late delivery gates (push/PR/merge)
        # when implementation already finished without an earlier workspace
        if work.current_phase not in (
            "approved",
            "ready_for_review",
            "completed",
        ):
            raise UserError(
                "Execution workspace preparation requires a Work Item in "
                "Approved, Ready for Review, or Completed (with an exact "
                "approved plan still bound)."
            )
