...
# After Analyzing, working analysis drafts must not stay open (e.g. Reviewed).
POST_ANALYSIS_PHASES = frozenset(
    {
        "planning",
        "awaiting_plan_approval",
        "approved",
...
ANALYSIS_WORKING_STATUSES = ("draft", "generated", "reviewed")
ANALYSIS_OPEN_PHASES = frozenset({"registered", "analyzing"})
...
class DevWorkItem(models.Model):
    ...
    def transition_lifecycle(
        ...
        # Past Analyzing: close leftover draft/generated/reviewed analyses.
        if new_phase in POST_ANALYSIS_PHASES:
            self._close_working_analyses(
                "Left Analyzing for %s — working analysis revisions closed"
                % new_phase
            )
        self._refresh_context_revision()
        return True

    def _close_working_analyses(self, reason=None):
        """Supersede draft/generated/reviewed analyses once work is past Analyzing.

        Accepted revisions stay. Later phases (planning, development, completed, …)
        must not keep an open Reviewed / Generated analysis looking actionable.
        """
        for work in self:
            working = work.analysis_ids.filtered(
                lambda item: item.status in ANALYSIS_WORKING_STATUSES
            )
            if working:
                super(type(working), working).write({"status": "superseded"})
...
class DevWorkAnalysis(models.Model):
    ...
    clipboard_result = fields.Text(
        string="Copyable Analysis Result",
        compute="_compute_clipboard_result",
        help="Formatted analysis and findings text for one-click clipboard copy.",
    )
    work_lifecycle_phase = fields.Selection(
        related="work_item_id.current_phase",
        string="Work Lifecycle Phase",
        readonly=True,
    )
    ...
    def _compute_clipboard_result(self):
        sections = (
            ("Problem Summary", "problem_summary"),
            ("Original Request", "original_request_summary"),
            ("Reproduction Context", "reproduction_context"),
            ("Current Behavior", "current_behavior"),
            ("Expected Behavior", "expected_behavior"),
            ("Technical Findings", "technical_findings"),
            ("Affected Modules / Files", "affected_components"),
            ("Risks", "risks"),
        ...
        for record in self:
            ...
            for title, field_name in sections:
                text = (record[field_name] or "").strip()
                if text:
                    parts.append("## %s\n%s" % (title, text))
            record.clipboard_result = "\n\n".join(parts)
    ...
    def action_mark_reviewed(self):
        """Human reviewed a generated/draft analysis before Accept."""
        self.ensure_one()
        if self.work_item_id.current_phase not in ANALYSIS_OPEN_PHASES:
            raise UserError(
                "Mark Reviewed is only available while Registered or Analyzing "
                "(current: %s)." % (self.work_item_id.current_phase or "unknown")
            )
        if self.status not in ("draft", "generated"):
            raise UserError("Only Draft or Generated analysis can be marked Reviewed.")
        super(DevWorkAnalysis, self).write({"status": "reviewed"})
        return True

    def action_accept(self):
        self.ensure_one()
        work = self.work_item_id
        # Mirror import / merge: a working analysis may advance Registered → Analyzing.
        if work.current_phase == "registered":
            work.transition_lifecycle(
                "analyzing",
                "Analysis acceptance advanced work into Analyzing",
            )
        if work.current_phase != "analyzing":
            raise UserError(
                "Analysis acceptance requires the Analyzing phase "
                "(current: %s). Use Start Analysis on the work item first."
                % (work.current_phase or "unknown")
            )
        if self.status not in ("draft", "generated", "reviewed"):
            raise UserError("Only a working analysis revision can be accepted.")
        old = work.analysis_ids.filtered(
        ...
        leftovers = work.analysis_ids.filtered(
            lambda r: r.status in ANALYSIS_WORKING_STATUSES and r != self
        )
        if leftovers:
            super(DevWorkAnalysis, leftovers).write({"status": "superseded"})
...
class DevWorkPlan(models.Model):
    ...
    clipboard_result = fields.Text(
        string="Copyable Plan Result",
        compute="_compute_clipboard_result",
        help="Formatted plan text for one-click clipboard copy.",
    )
    work_lifecycle_phase = fields.Selection(
        related="work_item_id.current_phase",
        string="Work Lifecycle Phase",
    ...
    def action_submit_for_approval(self):
        self.ensure_one()
        work = self.work_item_id
        # Mirror plan import: a complete draft may advance Analyzing → Planning.
        if work.current_phase == "analyzing":
            work.transition_lifecycle(
                "planning",
                "Plan submission advanced work into Planning",
                artifact=self,
            )
        if work.current_phase != "planning":
