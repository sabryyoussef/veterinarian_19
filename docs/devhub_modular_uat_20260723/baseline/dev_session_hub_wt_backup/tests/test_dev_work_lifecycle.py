...
class TestDevWorkLifecycle(TransactionCase):
    ...
    def test_accept_analysis_advances_registered_to_analyzing(self):
        """Accept from Registered must advance phase (no Invalid Operation)."""
        work = self._work()
        self._transition(work, "triage")
        self._transition(work, "registered")
        analysis = self._analysis(work)
        self.assertEqual(work.current_phase, "registered")
        analysis.action_accept()
        self.assertEqual(analysis.status, "accepted")
        self.assertEqual(work.current_phase, "analyzing")
        self.assertEqual(work.current_accepted_analysis_id, analysis)
        self.assertEqual(analysis.work_lifecycle_phase, "analyzing")

    def test_submit_plan_advances_analyzing_to_awaiting_approval(self):
        """Request Approval from Analyzing advances through Planning to the gate."""
        work = self._work()
        self._transition(work, "triage")
        self._transition(work, "registered")
        self._transition(work, "analyzing")
    ...
    def test_execution_workspace_prepare_allowed_after_ready_and_completed(self):
        """Late delivery: prepare workspace after implementation finished."""
        work = self._work()
        plan = self._approved_plan(work)
        self._transition(work, "implementing")
        with self.assertRaises(UserError):
            self._workspace_proposal(work)

        # Reach ready_for_review through the real delivery path (no SQL phase hacks).
        step = plan.step_ids[:1]
        step.write({"status": "in_progress"})
        step.write({"status": "done"})
        self._transition(work, "testing")
        ...
        report = self.env["dev.completion.report"].create(
            {
                ...
                "plan_id": plan.id,
                "original_request_summary": "Safe original request",
                "implemented_summary": "Implemented without early workspace.",
                "completed_steps_summary": "S1",
                "changed_components_summary": "dev_session_hub only",
                "repository_reference": self.repository.working_directory,
                "branch": "feature/late-prepare",
                "tests_summary": "TransactionCase passed.",
                "uat_status": "not_applicable",
                "known_limitations": "No production deployment.",
                "rollback_notes": "No deployment was performed.",
        ...
        # Second active proposal must fail closed.
        with self.assertRaises(UserError):
            self._workspace_proposal(work)
        ...
        task = self.env["project.task"].create(
            {
                "name": "Late prepare work package %s" % wp_id,
                "project_id": self.odoo_project.id,
                "op_backend_id": self.backend.id,
                "op_work_package_id": wp_id,
                "op_url": "https://openproject.test.invalid/work_packages/%s" % wp_id,
        ...
        work2 = self._work(wp_id=wp_id, task=task)
        report2 = self._approved_report(work2)
        self.assertEqual(work2.current_phase, "completed")
        workspace2 = self._workspace_proposal(work2)
        self.assertEqual(workspace2.plan_id, report2.plan_id)
        self.assertEqual(workspace2.state, "pending_confirmation")
        self.assertEqual(work2.current_phase, "completed")
