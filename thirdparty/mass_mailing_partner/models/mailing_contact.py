...
class MailingContact(models.Model):
    ...
    def _onchange_partner_mass_mailing_partner(self):
        if self.partner_id:
            ...
            if "title" in self.partner_id._fields and "title_id" in self._fields:
                self.title_id = self.partner_id.title
            company_name = False
            if self.partner_id.company_id:
                company_name = self.partner_id.company_id.name
            elif "company_name" in self.partner_id._fields:
    ...
    def _prepare_partner(self):
        ...
        if "title" in self.env["res.partner"]._fields and self.title_id:
            vals["title"] = self.title_id.id
        return vals
